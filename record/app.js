...
// 아래 둘은 일부러 다르다.
//  · 팀 기준이닝은 여럿을 평균 내는 값이라 한 사람이 흔들려도 묻힌다 → 친 사람은 다 담는다(1경기).
//  · 반대로 개인에게 "수지를 이 값으로 고치세요"라고 내미는 건 표본이 넉넉할 때만 한다(5경기).
// 그래서 5경기가 안 되는 사람도 팀 평균이닝·에버리지 계산에는 그대로 들어가되, 자기 칸은 '—' 로 남는다.
const REC_MIN_GAMES = 1;    // 팀 기준이닝을 낼 때 넣을 최소 경기수 — 한 경기라도 있으면 담는다
